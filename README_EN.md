...
## 🚀 Quick Start: Build and Use the RenderMark Library
...
### 3. Install RenderMark

After building this project with **Qt Creator**, enter the **build subdirectory** that contains `cmake_install.cmake` and run the install command:

```bash
# Note: not the build/ root, but the actual subdirectory generated by Qt Creator
cd build/Desktop_Qt_6_8_3_MSVC2022_64bit-RelWithDebInfo
cmake --install . --prefix "D:/temp"
```

> ⚠️ **Note**: `build/` itself is not the CMake build directory. The actual build directory is a subdirectory inside it (e.g. `Desktop_Qt_6_8_3_MSVC2022_64bit-RelWithDebInfo`). `cmake_install.cmake` lives in that subdirectory; running install against `build/` directly will fail with `Not a file: .../build/cmake_install.cmake`.
...
│   ├── cmake/RenderMark/         # find_package configuration
│   └── qml/RenderMark/           # QML module (qmldir + .qml + plugin)
└── include/RenderMark/           # C++ headers
```
...
In **Qt Creator**: go to **Projects → Build → Initial CMake parameters** and add:
...
> 📱 **Android**: You do **not** need to call `addImportPath` manually. The RenderMark QML module and `libRenderMarkplugin.so` are automatically bundled into the APK by Qt's deployment tool (`androiddeployqt`). The QML engine loads them directly from inside the APK at runtime.
...
Make sure the following DLLs can be found at runtime:
- `RenderMark.dll`
- `cmark-gfm.dll`
- `cmark-gfm-extensions.dll`

Recommended: in Qt Creator, go to **Projects → Run → Environment** and add `D:/temp/bin` to `PATH`:
```
...
- Android does not use DLLs; the equivalent is `.so` shared libraries (`libRenderMark.so`, `libcmark-gfm.so`, etc.)
- These `.so` files are automatically packaged into the APK's `libs/<abi>/` directory by `androiddeployqt` / Gradle
- **No manual path configuration is required** — Qt's Android deployment mechanism handles this automatically
...
[testmarkqml](https://github.com/yourname/testmarkqml) is a minimal external-project example demonstrating how to integrate RenderMark into a standalone Qt 6 project.
...
Window {
    ...
    RenderMark.RenderMark {
        ...
        "|------|------|-------------|\n" +
        "| Mark | class | Markdown parser |\n" +
        "| MarkNode | class | AST node |\n" +
        "| MarkTree | class | AST tree container |\n\n" +
        "> This is a blockquote.\n\n" +
        "---\n\n" +
        "**Use the button above to open a local .md file for testing.**"
    }
}
```
