...
`RenderMark` 不再内置 outline 联动；用 `onTreeReady` + `headingNodeCallback` 自行实现：
...
**说明**：
- `treeReady` 信号在 `tree` 变化时发出；连接到 `outlineView.rebuild()` 即可清空旧 heading 数据。
- `headingNodeCallback(item)` 在每个 heading 组件渲染完成时调用，把 `item` 作为它的 QML 实例传出，内部包含 `astNode` / `astStyle` 等属性。
- `MarkOutline.registerHeading(node, item)` 把一条 heading 加入大纲；`MarkOutline` 自身的 `headingClicked(node, item)` 信号让你拿到点击的 item，再传给 `RenderMark.scrollToHeading(item)` 即可滚动定位。
- 滚动时保留 20px 顶部边距。
